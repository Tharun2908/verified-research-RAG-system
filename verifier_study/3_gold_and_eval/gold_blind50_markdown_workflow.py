"""
backend/app/services/gold_blind50_markdown_workflow.py

Make blind-50 review human-readable and import labels back into JSONL.

Export:
  python -m app.services.gold_blind50_markdown_workflow export

Fill in the markdown:
  HUMAN_LABEL: SUPPORTED / UNSUPPORTED / ABSTENTION
  HUMAN_NOTES: optional

Import:
  python -m app.services.gold_blind50_markdown_workflow import-labels

This writes:
  data/distill_arxiv/gold_blind50_review_labeled.jsonl

It does not overwrite the original JSONL unless --overwrite-original is passed.
"""
...
def export_markdown(args: argparse.Namespace) -> None:
    ...
    print("\nOpen and edit only this markdown file while blind labeling.")
...
def parse_markdown_labels(path: Path) -> dict[str, dict[str, str]]:
    ...
    for i, m in enumerate(matches):
        ...
        label_match = re.search(r"\*\*HUMAN_LABEL:\*\*\s*([A-Za-z_ -]*)", section)
        ...
        label = ""
        if label_match:
            label = label_match.group(1).strip().upper().replace(" ", "_").replace("-", "_")

        notes = ""
        if notes_match:
            notes = notes_match.group(1).strip()

        labels[blind_id] = {"human_label": label, "human_notes": notes}
...
def import_labels(args: argparse.Namespace) -> None:
    ...
    if blank_labels:
        print("\nFill blank labels before running the audit comparison.")
